...
imgtags.forEach( imgtag => { // 각각의 각인이미지 를 변수 imgtag로 나눔
  imgtag.addEventListener("dragstart",()=>{ // 각 이미지를 드래그 시작했을때
    imgtag.classList.add("dragging"); // 각 이미지에 드래그중 이라는 클래스 추가
  });

  imgtag.addEventListener("dragend",()=>{ // 드래그가 끝나면
    imgtag.classList.remove("dragging"); // 드래그중 클래스 삭제
  });
});

renkins.forEach( renkin => { //각 이미지가 들어갈 구역을 나눔
  renkin.addEventListener("dragover", e =>{ // 드래그가 이미지 구역에 들어오면
...
function getDragAfterElement(target, x , y) { // 드래그중인 구역에 현재 마우스 위치 찾는 function
  const draggableElements = [ 
    ...target.querySelectorAll(".imgtag:not(.dragging)"),
  ];  // 구역안에 드래그 중이지 않은 각 객채들을 감지
  return draggableElements.reduce(  // 구역안의 드래그중인 객체와 안에 있던 객체의 위치를 비교하여 출력
    (closest, child) => {
      const box = child.getBoundingClientRect();
      const offsetx = x - box.left - box.width / 2; // 현재 위치의 정확한 x값을 세팅
      const offsety = y - box.top - box.height / 2; // 연재 위치의 정확한 y값을 세팅
      // console.log(offset);
      if (offsetx < 0 && offsetx > closest.offset) {  // 기존에 있던 객체와의 x값 비교
        if (offsety < 0 && offsety > closest.offset) { // 기존에 있던 객체와의 y값 비교
